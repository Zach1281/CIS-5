...
int main() 
{
    ...
    cin >> choice[0];
    while (choice[0] == 'Y' || choice[0] == 'y')//While loop for choice in starting the game
    {
        cin >> num[0];
        if (num[0] <= 12)
        {
        ...
        cin >> choice[1];
        while (choice[1] == 'Y' || choice[1] == 'y')//another while loop for the choice to continue and break up the rounds
        {
            for(int j=0;j<1;j++)
            {   
                dice[1] = (rand() % 12) + 1;
